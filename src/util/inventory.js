...
export const uploadFile = async (file) => {
  ...
  try {
      const response = await axios.post(`${process.env.REACT_APP_BASE_URL}/api/v1/inventories`, formData, {
          headers: {
...
export const fetchInventoryView = async (inventoryId) => {
  ...
  try {
    const response = await axios.patch(
      `${process.env.REACT_APP_BASE_URL}/api/v1/inventories/${inventoryId}/views`, 
...
export const deleteProduct = async (productId) => {
    const accessToken = localStorage.getItem("accessToken");
    const url =`${process.env.REACT_APP_BASE_URL}/api/v1/inventories/products/${productId}`;
